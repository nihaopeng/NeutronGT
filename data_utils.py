...
def save_model_output(args, score, global_ids, N,acc,prefix="full"):
    if args.batch_size >= N:
        path_score = f"full_batch_{args.dataset}.pt"
        print(f"full size:{N}")
    else:
    ...
    final_score = score_cpu[:, valid_mask_ids, :][:, :, valid_mask_ids]
    ...
    print(f"✅ [{prefix.upper()}] 已保存 Score ({final_score.shape}) 和 ID ({final_ids.shape}) to {path_score}")

    if torch.distributed.is_initialized():
        torch.distributed.barrier()

def analyze_global_recall_in_batch_scope(dataset_name, full_batch_size, mini_batch_size, top_ratio=0.1):
    # 1. 加载数据
    full_path = f"5916_batch_{dataset_name}.pt"
    mini_path = f"{mini_batch_size}_batch_{dataset_name}.pt"
    ...
    full_ids = full_data['final_ids'].tolist()
    # 获取 mini-batch 涉及的所有全局 ID 集合
    mini_ids_set = set(mini_data['final_ids'].tolist())

    N = full_score.size(0)

    # 2. 提取全局真正的 Top-K 边 (Ground Truth 标杆)
    full_score.fill_diagonal_(0)
    k_global = int(N * N * top_ratio) 
    _, topk_indices = torch.topk(full_score.view(-1), k_global)
    
    global_top_edges = []
    for idx in topk_indices.tolist():
        r, c = idx // N, idx % N
        global_top_edges.append((full_ids[r], full_ids[c]))

    # 3. 计算“落在 Batch 内”的比例
    # 只要边的两个端点都在 mini_ids_set 中，就算命中
    captured_count = 0
    for u, v in global_top_edges:
        if u in mini_ids_set and v in mini_ids_set:
            captured_count += 1

    # 4. 计算指标
    # 全局捕获率: 全局高分边中有多少比例的边，其两端节点都被采样进了这个 Batch
    global_recall = captured_count / len(global_top_edges) if len(global_top_edges) > 0 else 0
    acc_drop = full_data['acc'] - mini_data['acc']

    print(f"📊 采样覆盖分析: {dataset_name} (Batch Size: {len(mini_ids_set)})")
    print(f"✅ 全局高分边总数 (Top {top_ratio*100}%): {len(global_top_edges)}")
    print(f"🚀 落在 Batch 节点闭环内的全局高分边数: {captured_count}")
    print("-" * 30)
    print(f"🏆 结构捕获率 (Global Recall): {global_recall:.6f}")
    print(f"📉 准确率损耗 (Acc Drop): {acc_drop:.6f}")
    
    return global_recall, acc_drop