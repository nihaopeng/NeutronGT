...
def compute_graphormer_spatial_pos_only(ppr:tuple[list[torch.Tensor,torch.Tensor],torch.Tensor], partitions:list[torch.Tensor], num_nodes:int, max_dist:int=5):
    ...
    for partition in tqdm(partitions, desc="Computing spatial pos on induced subgraphs"):
        ...
        L = len(partition)
        if L == 0:
            spatial_pos_list.append(torch.zeros(0, 0, dtype=storage_dtype))
            continue
        ...
        dist_mat = np.full((L, L), max_dist + 1, dtype=np_storage_dtype)
        for src in range(L):
            dist_mat[src, src] = 0
            if max_dist == 0:
                continue
        ...
        spatial_pos_np = np.where(
            ...
            dist_mat + 1,
            0
        ).astype(np_storage_dtype, copy=False)
        spatial_pos_list.append(torch.from_numpy(spatial_pos_np))
