...
@torch.no_grad()
def evaluate_cpu(model, dataset, split_idx, eval_func, criterion, args, result=None, epoch=0):
    model.eval()
    ...
    # 构造全图的全局 IDs (从 0 到 N-1)
    num_nodes = x.shape[0]
    full_global_ids = torch.arange(num_nodes)
    
    adjs = []
    adjs.append(adjs_[0])
    for k in range(args.rb_order - 1):
        adjs.append(adjs_[k + 1])
    out, _, rt_score = model(x, adjs)
