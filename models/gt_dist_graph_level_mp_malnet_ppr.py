...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, x, attn_bias=None, edge_index=None, attn_type=None, mask=None, pruning_mask=None,
        ...
        x = self.local_attn(q, k, v, attn_bias, edge_index, attn_type, mask, pruning_mask)
...
class GT(nn.Module):
    def __init__(
        ...
        self.embedding_h = nn.Embedding(num_atoms, hidden_dim)
    ...
    def forward(self, x, in_degree_i, out_degree_i, edge_index, attn_type=None,
        ...
        x = self.embedding_h(x).sum(dim=-2)
