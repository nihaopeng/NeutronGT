...
def kernelized_softmax(query, key, value, kernel_transformation, projection_matrix=None, edge_index=None, tau=0.25, return_weight=True):
    ...
    if return_weight: # query edge prob for computing edge-level reg loss, this step requires O(E)
        ...
        # --- 构建 [H, N, N] 矩阵 ---
        # 创建全零矩阵
        full_attn_matrix = torch.zeros((H, N, N))
        # 使用 scatter_ 填充。注意：A_weight 是 [E, H]，我们需要填入 [H, row, col]
        # 转换 A_weight 为 [H, E] 方便索引
        if A_weight.dim() == 3:
            A_weight = A_weight.squeeze(0)  # 变成 [E, H]
        A_weight_t = A_weight.t() 
        for h in range(H):
            # 针对每个头，将边权重填入对应的 (end, start) 位置
            # 注意：NodeFormer 这里的 end 对应 query，start 对应 key
            full_attn_matrix[h, end, start] = A_weight_t[h]
        return z_output, A_weight, full_attn_matrix
...
def kernelized_gumbel_softmax(query, key, value, kernel_transformation, projection_matrix=None, edge_index=None,
    ...
    if return_weight: # query edge prob for computing edge-level reg loss, this step requires O(E)
        ...
        # --- 构建 [H, N, N] 矩阵 ---
        # 创建全零矩阵
        full_attn_matrix = torch.zeros((H, N, N),device=A_weight.device)
        # 使用 scatter_ 填充。注意：A_weight 是 [E, H]，我们需要填入 [H, row, col]
        # 转换 A_weight 为 [H, E] 方便索引
        if A_weight.dim() == 3:
            A_weight = A_weight.squeeze(0)  # 变成 [E, H]
...
class NodeFormerConv(nn.Module):
    ...
    def forward(self, z, adjs, tau):
        ...
        if self.use_gumbel and self.training:  # only using Gumbel noise for training
            z_next, weight, full_attn_matrix = kernelized_gumbel_softmax(query,key,value,self.kernel_transformation,projection_matrix,adjs[0],
                                                  self.nb_gumbel_sample, tau, self.use_edge_loss)
        else:
            z_next, weight, full_attn_matrix = kernelized_softmax(query, key, value, self.kernel_transformation, projection_matrix, adjs[0],
                                                tau, self.use_edge_loss)

        rt_score = full_attn_matrix
        ...
        if self.use_edge_loss: # compute edge regularization loss on input adjacency
            ...
            return z_next, link_loss, rt_score
    ...
    def forward(self, x, adjs, tau=1.0):
        ...
        for i, conv in enumerate(self.convs):
            if self.use_edge_loss:
                z, link_loss,score = conv(z, adjs, tau)
        ...
        if self.use_edge_loss:
            return x_out, link_loss_, rt_score
        else:
            return x_out