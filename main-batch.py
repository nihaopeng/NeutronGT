...
from data_utils import save_model_output,analyze_global_recall_in_batch_scope
    ...
    for epoch in range(args.epochs):
        ...
        for i in range(num_batch):
            ...
            for k in range(args.rb_order - 1):
                ...
                adjs_i.append(edge_index_i.to(device))
            optimizer.zero_grad()
            out_i, link_loss_, rt_score = model(x_i, adjs_i, args.tau)
            if epoch == args.epochs - 1 and i == 0: # 只保存最后一个 epoch 的第一个 batch
                print(f"rt_score shape: {rt_score.shape}")
                save_model_output(
                    args=args,
                    score=rt_score,
                    global_ids=idx_i,
                    N=n,
                    acc=0,
                    prefix=str(args.batch_size)
                )
                if args.batch_size < n:
                    analyze_global_recall_in_batch_scope(args.dataset,n,args.batch_size)
            # print(f"rt_score shape: {rt_score.shape}")
            if args.dataset in ('yelp-chi', 'deezer-europe', 'twitch-e', 'fb100', 'ogbn-proteins'):
        ...
        if epoch % 9 == 0:
            result = evaluate_cpu(model, dataset, split_idx, eval_func, criterion, args, epoch=epoch)
