...
class MalNet(InMemoryDataset):
    ...
    def raw_file_names(self) -> List[str]:
        current_directory = self.root
        raw_path = osp.join(current_directory, 'raw')
        
        # 如果 raw 目录不存在，返回空列表让 PyG 知道需要下载
    ...
    def download(self):
        ...
        split_dir = osp.join(self.root, 'split_info')
        ...
        # 下载图数据
        graph_tar_path = osp.join(self.root, 'malnet-graphs-tiny.tar.gz')
        if not osp.exists(graph_tar_path):
            print(f"Downloading MalNet graph data from {self.url}...")
            urllib.request.urlretrieve(self.url, graph_tar_path)
            print("Download complete. Extracting...")
            import tarfile
            with tarfile.open(graph_tar_path, 'r:gz') as tar:
                tar.extractall(raw_dir)
            print("Graph data extracted.")

        # 下载划分信息
        split_zip_path = osp.join(self.root, 'split_info_tiny.zip')
        if not osp.exists(split_zip_path):
            print(f"Downloading MalNet split info from {self.split_url}...")
            urllib.request.urlretrieve(self.split_url, split_zip_path)
            print("Download complete. Extracting...")
            with zipfile.ZipFile(split_zip_path, 'r') as zip_ref:
                zip_ref.extractall(self.root)
            # 移动文件到 split_info 目录
            extracted_split_dir = osp.join(self.root, 'split_info_tiny')
            if osp.exists(extracted_split_dir):
                for f in os.listdir(extracted_split_dir):
                    src = osp.join(extracted_split_dir, f)
                    dst = osp.join(split_dir, f)
    ...
    def process(self):
        ...
        graph_count = 0
        # 获取原始路径
        raw_path = self.raw_paths[0] if self.raw_paths else None
        if raw_path is None:
            raise ValueError("No raw paths found")
        
        # 获取类别目录列表，并按名称排序以保证顺序一致
        directories = sorted([d for d in os.listdir(raw_path) if os.path.isdir(os.path.join(raw_path, d))])
        print(f"Class directories: {directories}")
        
        # 创建类别名到索引的映射
        class_to_idx = {name: idx for idx, name in enumerate(directories)}
        ...
        for class_name in directories:
            ...
            print(f"Processing class {class_name} (y={y})")
            
            # 递归搜索所有 .edgelist 文件
            filenames = glob.glob(os.path.join(class_path, '**/*.edgelist'), recursive=True)
            print(f"Found {len(filenames)} edgelist files in {class_name}")
